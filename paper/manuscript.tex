...
\begin{abstract}
AI-powered coding agents are increasingly used for software development tasks, yet their behavior across different task types remains insufficiently characterized. This paper presents a small-scale empirical case study of an agentic coding setup using Claude Code as the orchestration environment with a DeepSeek-based backend. We evaluate the setup on 50 software development tasks covering algorithm implementation, bug fixing, refactoring, and test writing across JavaScript, TypeScript, Go, Python, and JSX. In the primary single-run evaluation, within this small-scale benchmark, the agent completed all 50 tasks and passed all 414 predefined test assertions. To assess run-to-run stability, we further repeated six representative tasks five times each. The core 18 tasks achieved stable success across all repeated runs, whereas a distributed-lock bug-fixing task succeeded in only 3 out of 5 runs, revealing substantial variability for complex concurrent logic. These findings suggest that in our benchmark, agentic coding systems performed strongly on the tested tasks, but single-run evaluation may overestimate reliability for concurrency-heavy or distributed-system tasks. We discuss threats to validity including benchmark size. All supplementary materials are publicly available on GitHub. We discuss threats including training-data contamination, absence of human baselines, and limitations of test-based evaluation.
\end{abstract}
...
\begin{enumerate}
 \item A systematically designed benchmark of 50 software development tasks spanning four task types and three difficulty levels, implementing a balanced evaluation framework for AI coding agents.
 \item A comprehensive empirical evaluation of Claude Code with deepseek-v4-flash, achieving 100\% task pass rate and 100\% test assertion pass rate across 414 tests in five programming languages in single-run evaluation, complemented by a variability analysis across 30 repeated runs.
...
We designed a benchmark suite of 18 core software development tasks (Table~I) organized along two dimensions, forming our primary evaluation benchmark. To assess scalability, we extended the benchmark to 50 tasks total, with the additional 32 tasks described in the supplementary materials. The core results in this paper are based on the 18-task benchmark, which received full repeated-measures evaluation.: task type and difficulty level. The four task types represent core software engineering activities:
...
\begin{itemize}[leftmargin=*]
 ...
 \item 			extbf{Language Robustness:} Consistency of code generation quality across JavaScript, TypeScript, Go, Python, and JSX.
...
Within our small-scale benchmark, the Claude Code framework achieved a 100\% task pass rate, successfully completing all 50 tasks across the four categories and three difficulty levels. A total of 414 individual test assertions were executed, all of which passed (100\%). Table~\ref{tab:overall_results} summarizes the aggregate results.
...
The complete experimental data, including all 50 task specifications, generated code, and 30 repeated-run records, is available in the supplementary materials (GitHub: https://github.com/Xiaobai233/ai-coding-agent-evaluation).
...
\subsubsection{Internal Validity}
			...
			extbf{Task selection bias:} The 50 tasks were designed by the researchers and may not represent the full distribution of challenges in industrial software development. There is a risk that the selected tasks align particularly well with the agent's capabilities, leading to inflated performance estimates.
