...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Hardware and Software:} Experiments were conducted on a standard development workstation running Ubuntu 22.04 with an Intel i7 processor and 32 GB RAM. The Claude Code agent operated with default settings (no custom configuration files or environment overrides beyond the API proxy URL).
...
\subsubsection{Semantic Brittleness in Test-Code Alignment}

Although all five H2 implementations implement correct distributed lock semantics---each uses Lua-based atomic identity verification, a watchdog goroutine for automatic lock renewal, and a unique per-instance identifier---the functional test suite (\texttt{lock\_test.go}) directly accesses the struct field \texttt{lock2.value} in the identity verification test. Only Run~1 preserved this exact field name (\texttt{value}); Runs~2--5 used alternative names (\texttt{id}, \texttt{token}, \texttt{ownerID}, and \texttt{lockID}, respectively). While the alternative naming does not affect the underlying lock correctness---each implementation correctly compares the owner identity in Lua scripts---the field name mismatch causes a compilation error in \texttt{TestLockIdentity}, which expects the \texttt{value} field. This represents a form of \emph{semantic brittleness}: the agent correctly implements the required behavior but does not consistently preserve the exact API surface expected by the pre-existing test suite. This finding suggests that test suites for agent-evaluation should prefer interface-based or behavioral verification over direct struct field access.

\subsubsection{Error Handling and Timing Sensitivity}

Beyond the naming mismatch, all five runs share two error-handling gaps that contribute to runtime instability. First, all implementations discard Lua script execution errors via the blank identifier (\texttt{\_ = script.Run(...)}), meaning transient Redis failures---such as connection timeouts during lock renewal---are silently ignored, potentially leading to premature lock expiry under load. Second, the watchdog goroutines use \texttt{context.Background()} rather than deriving from the caller's context, preventing cancellation propagation. These gaps are benign in isolation but compound under the concurrent stress test (\texttt{TestConcurrentLock}), which launches 30 goroutines competing for the same lock. The goroutine scheduling order and Redis operation timing introduce non-determinism: if system load delays the watchdog's first renewal cycle past the TTL, the lock is lost, and another goroutine's \texttt{SetNX} can acquire it, violating mutual exclusion expectations.

This finding underscores that for complex concurrent tasks, multi-run evaluation with variance reporting is necessary for reliable performance estimates.
...
The 100\% success rate achieved by Claude Code across all 18 tasks provides preliminary evidence of performance on the selected tasks. This result is particularly meaningful given the deliberate diversity of our benchmark, which spans fundamental algorithmic reasoning, diagnostic debugging, software design and architecture, and testing methodology---skills that collectively represent the core competencies expected of professional software developers.
...
The consistent performance across task types suggests that current AI coding agent capabilities may be more evenly distributed across task categories than observed in earlier code generation benchmarks. This contrasts with earlier findings~\cite{chen2021evaluating} that showed significant performance variation across problem categories in code generation benchmarks. The improvement may reflect advances in both base model capabilities and the agentic framework's ability to adapt its problem-solving strategy to different task requirements.
...
To provide external context for interpreting our results, we reference the HumanEval benchmark \cite{chen2021evaluating} as a loose external reference. HumanEval reports a pass@1 rate of 96\% from professional human developers on 164 programming problems. We note that HumanEval and our benchmark differ in task composition, difficulty distribution, and evaluation protocols; therefore, this comparison should be interpreted as a rough contextual reference rather than a direct performance comparison. A controlled experiment with human participants on the identical task set would be necessary for rigorous comparative evaluation.
