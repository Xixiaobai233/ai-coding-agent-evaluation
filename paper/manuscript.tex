...
\begin{enumerate}
 ...
 \item A three-layer analysis framework (semantic correctness, interface compatibility, temporal stability) for understanding AI code reliability beyond single-run pass/fail, with a case study demonstrating all three layers.\item A variance index for quantifying run-to-run reliability, supported by empirical data from 30 repeated runs.
\end{enumerate}
...
\[VI = rac{1}{n}\sum_{i=1}^{n}(x_i - ar{x})^2\]
where $x_i$ is the tests passed in the $i$-th run and $ar{x}$ is the mean. A low VI indicates stable performance; a high VI flags tasks where single-run results may be misleading.
