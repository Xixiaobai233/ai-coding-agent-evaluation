...
The Go test suite accesses the struct field \texttt{lock2.value} in \texttt{TestLockIdentity} and \texttt{lock.value} in \texttt{TestLockRenewal}. Only Run~1 preserved this exact field name; Runs~2--5 used alternative names (\texttt{id}, \texttt{token}, \texttt{ownerID}, \texttt{lockID}), causing three compilation errors per run.

This naming inconsistency is not a random coincidence but a systematic consequence of how LLMs generate code. Unlike a human developer who reads the existing codebase, maintains a working memory of identifier conventions, and deliberately reuses established names, the LLM independently samples each identifier from its prior distribution at generation time. Across five independent invocations, the five field names (\texttt{value}, \texttt{id}, \texttt{token}, \texttt{ownerID}, \texttt{lockID}) represent five independent draws from the same semantic neighborhood---all synonyms for ``unique identifier.'' Prior work in LLM output variability has documented analogous patterns in natural language generation~\cite{brown2020language}, but our results demonstrate that this phenomenon directly impacts code integration: the code is semantically correct but structurally incompatible.

We further note that even Run~1---which matches the expected field name---fails an additional identity verification test (\texttt{TestLockIdentity}) due to a subtle concurrency bug in the unlock logic: the Lua script does not correctly compare the lock value before deletion, causing \texttt{lock1.Unlock} to delete \texttt{lock2}'s lock. This indicates that field name consistency is necessary but not sufficient for correctness; the underlying behavioral semantics of the unlock operation must also align with the test's expectations. Together, these observations suggest that LLM-generated code carries an \emph{integration tax}---a systematic cost arising from the stateless generation paradigm---that conventional pass/fail metrics do not capture.

\textbf{Layer 3 --- Temporal Stability and Code Quality (Cross-Cutting).}
All five runs share two identical error-handling gaps, despite using different struct field names and code organization:

\begin{itemize}
  \item \textbf{Error swallowing:} Lua script execution uses the blank identifier (\texttt{\_ = script.Run(...)}), silently discarding all errors from Redis operations. If a transient network failure or Redis cluster reconfiguration causes a script execution failure, the lock operation silently fails---potentially leaving the lock in an inconsistent state (e.g., acquired without a watchdog, or released without clearing the Redis key).
  \item \textbf{Non-propagating cancellation:} Watchdog goroutines use \texttt{context.Background()} as their context root. Because \texttt{context.Background()} is never cancelled, these goroutines cannot respond to cancellation signals from the caller. If the application's main context is cancelled (e.g., during graceful shutdown), the watchdog continues running, potentially preventing lock cleanup and causing resource leaks.
\end{itemize}

These are not isolated mistakes but instances of a broader pattern in LLM-generated concurrent code: the model optimizes for locally correct execution paths while neglecting cross-cutting concerns such as error propagation, cancellation, and resource lifecycle management. This pattern is observable because LLMs are trained on code examples that often simplify error handling for pedagogical clarity, and the model reproduces these simplifications as production patterns. The two gaps compound under the concurrent stress test (30 goroutines contending for the same lock): if system load delays the watchdog's first renewal past the TTL, the lock expires, mutual exclusion is violated, and multiple goroutines acquire the lock simultaneously---precisely the behavior observed in Run~1, where 3 goroutines succeeded instead of the expected maximum of 2.

To assess the prevalence of such patterns, we propose a systematic \emph{AI code quality checklist} for concurrent code, derived from the H2 case:

\begin{itemize}
  \item \textbf{Error handling:} Are all non-trivial function calls' return values checked? Are errors propagated or explicitly acknowledged?
  \item \textbf{Cancellation:} Are goroutine contexts derived from the caller's context? Can long-running operations be cancelled?
  \item \textbf{Resource cleanup:} Are goroutines guaranteed to terminate? Are channels cleaned up on all exit paths?
  \item \textbf{Idempotency:} Can operations be safely retried? Are there guard conditions preventing double-execution?
\end{itemize}

These findings demonstrate the complementary value of the three-layer model: static verification passes for all runs, but the framework not only identifies interface brittleness (L2) and latent reliability risks (L3) that pass/fail metrics miss, but also reveals systematic patterns (stateless generation, cross-cutting neglect) that inform the design of more robust evaluation protocols for AI-generated concurrent code.
...
At L2 (interface compatibility), we identified a failure mode qualitatively different from traditional coding errors. The H2 implementations failed the Go test suite due to struct field naming mismatches---only Run~1 compiled, and it still failed an identity verification test due to a subtle concurrency bug. Runs~2--5 failed to compile due to field name differences. This \emph{semantic brittleness} reflects a fundamental characteristic of LLM generation: each invocation independently samples from the model's prior distribution without awareness of interface conventions in the consuming code. A human developer, by contrast, reads the existing codebase, maintains a working memory of identifier choices, and deliberately reuses established names across edits. The stateless generation paradigm of current LLMs inherently lacks this context maintenance capability, producing code that is semantically valid but structurally incompatible.

This observation carries implications for evaluation methodology: it suggests that a single ``pass'' verdict may mask substantial integration costs that only surface when the generated code is composed with existing codebases. We propose that evaluations of LLM-generated code should supplement functional correctness metrics with an \emph{interface consistency score}---measuring the proportion of identifier names, function signatures, and type definitions that match the surrounding codebase across repeated generations. Such a metric would quantify the integration tax that current pass/fail evaluations ignore.

At L3 (temporal stability), while static verification VI is 0.00 across all six tasks, post-hoc Go test execution revealed that Run~1 of H2 allowed 3 of 30 goroutines to acquire a lock designed for single ownership, indicating brief mutual exclusion violations under concurrent load. All five runs share identical error-handling gaps (discarded Lua errors via blank identifier, watchdog contexts not derived from caller context). The consistency of these gaps across five independent generations---despite varying identifier names and code structure---suggests they are not coincidental bugs but manifestations of a systematic pattern in LLM-generated concurrent code: the model optimizes for locally correct execution paths while systematically neglecting cross-cutting concerns (error propagation, cancellation, resource lifecycle). We term this phenomenon \emph{cross-cutting neglect} and argue that it represents a fundamental reliability risk in AI-generated concurrent systems. To assess its prevalence, we propose a code quality checklist (Section~\ref{sec:h2_detail}) and recommend that concurrent code generated by LLMs receives additional human review focused on error handling, context propagation, and resource cleanup---dimensions where the model's training signal is weakest.
