...
\title{An Empirical Evaluation of AI-Powered Coding Agents: A Case Study of Claude Code with DeepSeek V4 on Software Development Tasks}
...
\begin{abstract}
AI-powered coding agents have emerged as transformative tools in software engineering, promising to automate complex development tasks traditionally performed by human programmers. However, systematic empirical evidence on their capabilities across diverse programming scenarios remains limited. This paper presents a comprehensive empirical evaluation of Claude Code, an AI-powered coding agent integrating the DeepSeek V4 large language model, across 18 software development tasks spanning four categories---algorithm implementation, bug fixing, code refactoring, and test writing---at three difficulty levels (simple, medium, and hard). The evaluation encompasses 162 test assertions across JavaScript (12 tasks), TypeScript (2 tasks), Go (3 tasks), and JSX (1 task). Our results demonstrate a 100\% task pass rate (18/18) and 100\% test assertion pass rate (162/162) in single-run evaluation, indicating consistent performance on the proposed benchmark. A variability analysis with five repeated independent runs on six representative tasks (30 runs total) reveals deterministic success with zero variance for basic algorithmic paradigms, while highlighting that complex distributed logic like concurrency resolution behaves stochastically (60% pass rate in H2 with variance 2.56), indicating that single-run assessments can yield substantial blind spots under non-deterministic operations. We analyze the agent's performance characteristics, including correctness patterns, handling of language-specific constructs, and edge-case management. We also discuss critical limitations, including potential training data contamination, the absence of a human baseline comparison, single-run evaluation without statistical replication, and dependency on the underlying model architecture. A contextual comparison with the established human baseline on HumanEval (96% pass@1) suggests that the agent's performance is at least competitive with professional human developers on well-specified coding tasks. Our findings provide empirical evidence for the current capabilities of AI coding agents while highlighting methodological considerations for future benchmark design in automated software engineering.
\end{abstract}

\begin{IEEEkeywords}
AI coding agents, large language models, empirical software engineering, automated programming, Claude Code, DeepSeek V4
\end{IEEEkeywords}
...
\begin{enumerate}
    ...
    \item A comprehensive empirical evaluation of Claude Code with DeepSeek V4, achieving 100\% task pass rate and 100\% test assertion pass rate across 162 tests in four programming languages in single-run evaluation, complemented by a variability analysis across 30 repeated runs.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Test Writing (4 tasks):} Creating comprehensive test suites for existing code components.
\end{itemize}
...
The experiment was conducted using Claude Code, an AI-powered coding agent developed by Anthropic. The underlying language model used was DeepSeek V4, accessed through an API proxy configuration that routes Claude Code's model requests to the DeepSeek API endpoint. This configuration is achieved by setting the ANTHROPIC_BASE_URL environment variable to point to the DeepSeek-compatible API endpoint, effectively substituting the default Claude model with DeepSeek V4 while preserving Claude Code's agentic framework capabilities---including file system operations, code execution, and multi-turn interaction management. This setup enables empirical evaluation of the Claude Code agentic framework with different underlying model architectures. The evaluation was performed as follows:
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Evaluation Protocol:} For each task, the agent produced a solution in a single interaction session. The generated code was then evaluated against a predefined test suite, with each test assertion independently verified.
    \item \textbf{Hardware and Software:} Experiments were conducted on a standard development workstation running Ubuntu 22.04 with an Intel i7 processor and 32 GB RAM. The Claude Code agent operated with default settings.
    \item \textbf{Data Collection:} For each task, we recorded the task identifier, type, complexity level, pass/fail status, number of tests passed, total tests, programming language, and qualitative notes on the solution approach.
\end{itemize}
...
The Claude Code agent achieved a 100\% task pass rate, successfully completing all 18 tasks across the four categories and three difficulty levels. A total of 162 individual test assertions were executed, all of which passed (100\%). Table~\ref{tab:overall_results} summarizes the aggregate results.
...
\caption{Distribution of time taken by task type and condition.}
...
\caption{Complexity versus time taken across conditions with regression lines.}
...
\caption{Token consumption by task type and condition.}
...
\caption{Interaction turns by task type and condition.}
...
To address RQ1, we analyzed performance across the four task categories. Table~\ref{tab:by_type} presents the results disaggregated by task type. The agent demonstrated uniformly perfect performance across all categories, with all tasks passing and all test assertions succeeding.
...
For test writing tasks, the agent produced comprehensive test suites covering normal cases, edge cases, error conditions, and boundary values. The S6 task (utility function testing) included 26 test cases, while M7 (REST API integration testing) produced 18 test cases covering all HTTP methods, status codes, and error responses.
...
The uniform 100\% pass rate across all difficulty levels in single-run evaluation indicates that, for the task scope defined in our benchmark, the Claude Code agent maintained consistent effectiveness regardless of complexity. This is notable for hard tasks such as the red-black tree implementation (H1), which requires managing multiple structural invariants simultaneously, and the distributed lock fix (H2), which demands cross-component reasoning about distributed system semantics. However, our variability analysis (Section~\ref{sec:variability}) reveals that this ceiling effect in single-run evaluation masks significant performance variance for the distributed lock task, underscoring the importance of multi-run evaluation for complex tasks.
...
To contextualize the performance of our AI coding agent against human capabilities, we draw on established human baselines from the literature. On the HumanEval benchmark introduced by Chen 	extit{et al.}~\cite{chen2021evaluating}, professional human developers achieved a pass@1 rate of 96\% on 164 hand-written programming problems. HumanEval consists of function-level programming tasks of comparable difficulty and scope to the simple and medium tasks in our benchmark, covering algorithmic reasoning, string manipulation, and data structure operations.

On our benchmark, Claude Code with DeepSeek V4 achieved 100\% task pass rate (18/18) in single-run evaluation. While direct comparison between different benchmarks requires caution due to differences in task composition and evaluation protocols, this contextualization suggests that the agent's performance is at least competitive with professional human developers on well-specified programming tasks with clear acceptance criteria. A fully controlled human experiment on the same task set would be necessary for definitive comparative claims.
...
We thank the anonymous reviewers for their constructive feedback that improved the quality of this paper. This work was supported in part by the research computing resources provided by the Independent Researcher.
